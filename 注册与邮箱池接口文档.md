...
| `GET /api/external/pool/accounts` | 查看安全邮箱池库存，支持标签筛选 | 可选 |
...
`POST /api/external/pool/claim-random` 和 `claim-domain` 支持可选
`tags` 或 `include_tags`。账号必须同时拥有所有传入标签。带标签过滤时，服务不会回退到没有标签模型的临时邮箱池。
...
`POST /api/external/pool/claim-complete` 支持可选 `add_tags` 和
`remove_tags`。标签变更必须绑定有效的 claim 归属校验。
...
`GET /api/external/pool/accounts` 返回安全库存字段，支持 `provider`、`pool_status`、`email_domain`、`include_tags`、`exclude_tags`、`limit`。
...
`pool` 字段会声明 external pool 是否开启，以及是否支持库存查询、标签筛选、完成时标签变更。
